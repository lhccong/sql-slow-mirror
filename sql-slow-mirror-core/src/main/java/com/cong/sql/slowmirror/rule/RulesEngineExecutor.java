...
public class RulesEngineExecutor {
    ...
    public static boolean refresh() {
        ...
        properties.forEach((key, value) -> {
            ...
            if (keyArr.length == 2) {
                //获取规则内容,如果没有则创建一条规则,并将它放入 ret 中
                ...
                switch (RuleFieldEnum.valueOf(keyArr[1].toUpperCase())){
                    ...
                    //减分 配置到评分规则对象中
                    case SCORE: scoreRule.setScoreDeduction(NumberUtils.toInt(value.toString()));break;
                    //原因 配置到评分规则对象中
                    case REASON: scoreRule.setReason(value.toString());break;
                    //建议 配置到评分规则对象中
                    case SUGGESTION: scoreRule.setSuggestion(value.toString());break;
                    //是否严格执行 配置到评分规则对象中
                    case STRICT: scoreRule.setStrict(Boolean.valueOf(value.toString()));break;
                }
